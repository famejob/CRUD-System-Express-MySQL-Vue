...
const checkDuplicateEmail = async () => {
    try {
        const response = await axios.post(`http://localhost:8080/email/${customerID}`, { email: customer.email });
...
const checkDuplicatePhone = async () => {
    try {
        const response = await axios.post(`http://localhost:8080/phone/${customerID}`, { phone: customer.phone });
        if (response.data.exists) {
...
<template>
    <div class="container">
        <h1 class="my-2 text-center">Edit Customer</h1>
        <div class="mt-2">
